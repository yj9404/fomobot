# FomoBot

> 이미 오른 것들의 명예의 전당

---

## 이게 뭔가요?

FomoBot은 **"그때 샀어야 했는데..."** 하는 후회를 데이터로 보여주는 서비스입니다.

그 기회를 놓쳤다는 걸 알면서도 "에이, 나도 알고 있었어"라고 합리화하게 되는 그 순간 — FomoBot은 실제로 얼마나 올랐는지, 그리고 그때 잡았다면 지금 얼마였는지를 냉정하게 알려줍니다.

투자 조언이 아닙니다. FomoBot은 지나간 걸 보여줄 뿐이에요.

---

## 주요 기능

### 기간별 급등 랭킹
주식·부동산 등 여러 자산군에서 특정 기간 동안 가장 많이 오른 항목들을 순위로 보여줍니다.

| 기간 | 설명 |
|------|------|
| 전일 | 어제 대비 오늘 |
| 7일 | 일주일 상승률 |
| 30일 | 한 달 상승률 |
| 90일 | 분기 상승률 |
| 1년 | 연간 상승률 |
| 5년 | 5년 장기 상승률 |

각 종목에는 상승률 외에도 **변동성(σ)**, **지수 대비 초과수익**, **MDD(최대낙폭)** 정보가 함께 표시됩니다.

### 백테스트: 그때 샀다면?
랭킹에 오른 종목을 클릭하면 **"그 시점에 샀다면 지금 수익률이 얼마였는가"** 를 바로 확인할 수 있습니다. 올랐으면 "안 샀죠? 그럴 줄 알았어요."라고 하고, 내렸으면 "안 사길 잘했네요, 이번만큼은."이라고 합니다.

### 저유동성 필터링
시총이 너무 낮거나 거래대금이 없는 동전주·저유동성 항목은 자동으로 제외합니다. 진짜 오른 것만 봐야 제대로 된 후회를 할 수 있으니까요.

---

## 어떻게 동작하나요?

각 자산군별로 서버가 자동으로 데이터를 수집하고 랭킹을 계산합니다. 주식의 경우 수정주가를 사용하기 때문에 액면분할이나 배당이 수익률을 왜곡하지 않습니다.

---

## 주의사항

- **투자 조언이 아닙니다.** FomoBot은 과거 데이터를 보여주는 도구일 뿐이며, 미래 수익을 보장하지 않습니다.
- 백테스트 결과는 **생존 편향**이 있습니다. 상장폐지된 종목은 포함되지 않기 때문에 실제 수익률보다 좋게 보일 수 있습니다.
- 과거의 급등이 미래에도 반복된다는 보장은 없습니다.

---

## 지원 자산군

**주식**
- 🇰🇷 KOSPI (한국거래소)
- 🇺🇸 NASDAQ

**부동산**
- 🇰🇷 아파트 실거래가 (국토교통부 공개 데이터)

한국어 · English 양국어 지원
