...
+동명 지역명이면 시/도) + 발행일이 lookback 내" 규칙만 사용한다
(realestate.services.naver_news.filter_relevant_articles).

"중구"처럼 여러 시/도에 동시 존재하는 지역명은 시/도를 함께 요구하지 않으면
다른 도시의 동명 지역 기사가 오탐될 수 있다 — batch.regions.DUPLICATE_GU_NAMES/
get_duplicate_dong_names_sync로 판정하고, 매칭되면 검색 쿼리·필터 양쪽에
시/도를 추가한다(batch.regions.SIDO_NAME_VARIANTS).
"""

import logging
from datetime import date, timedelta

from realestate.batch.regions import DUPLICATE_GU_NAMES, SIDO_NAME_VARIANTS, get_sido
from realestate.config import settings
from realestate.db.crud import (
    delete_expired_region_news_sync,
    get_duplicate_dong_names_sync,
...
def collect_region_news() -> int:
    ...
    with SyncSessionLocal() as session:
        ...
        duplicate_dong_names = get_duplicate_dong_names_sync(session)
        ...
        for (sigungu_code, eupmyeondong), target in dong_targets.items():
            ...
            sido = get_sido(sigungu_code)
            key = region_key(sigungu_code, eupmyeondong)
            label = f"{sigungu_name} {eupmyeondong}"
            is_dup_dong = eupmyeondong in duplicate_dong_names
            sido_variants = SIDO_NAME_VARIANTS.get(sido) if (is_dup_dong and sido) else None
            try:
                query = (
                    f"{sido} {sigungu_name} {eupmyeondong} 아파트"
                    if sido_variants else f"{sigungu_name} {eupmyeondong} 아파트"
                )
                raw = search_news(query, display=100)
                articles = filter_relevant_articles(
                    raw,
                    target_name=eupmyeondong,
                    window_start=window_start,
                    window_end=window_end,
                    also_require_sido=sido_variants,
                )
        ...
        for sigungu_code, sigungu_name in gu_needs_fallback.items():
            ...
            is_dup_gu = sigungu_name in DUPLICATE_GU_NAMES
            sido_variants = SIDO_NAME_VARIANTS.get(sido) if (is_dup_gu and sido) else None
            try:
                query = (
                    f"{sido} {sigungu_name} 부동산" if sido_variants else f"{sigungu_name} 부동산"
                )
                raw = search_news(query, display=100)
                articles = filter_relevant_articles(
                    raw,
