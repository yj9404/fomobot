...
def aggregate_all_sigungu_complex(deal_yms: list[str]) -> None:
    ...
    valid_sigungu_codes = [sg["code"] for sg in SUDOGWON_SIGUNGU]
    ...
    with SyncSessionLocal() as session:
        sql = text("""
            ...
            WHERE deal_ym IN :deal_yms
              AND sigungu_code IN :sigungu_codes
              AND price_per_sqm > 0
            ORDER BY deal_date ASC
        """)
        sql = sql.bindparams(
            bindparam("deal_yms", expanding=True),
            bindparam("sigungu_codes", expanding=True),
        )
        result = session.execute(
            sql, {"deal_yms": deal_yms, "sigungu_codes": valid_sigungu_codes}
        )
        rows = [dict(r._mapping) for r in result]
    ...
    for row in agg.itertuples(index=False):
        ...
        if row.sigungu_code not in records_by_sigungu:
            records_by_sigungu[row.sigungu_code] = []

        records_by_sigungu[row.sigungu_code].append(
            {
                "complex_key": key,
                "sigungu_code": row.sigungu_code,
                "eupmyeondong": str(meta["eupmyeondong"])[:100],
    ...
    for sigungu_code in valid_sigungu_codes:
        if sigungu_code in records_by_sigungu:
            try:
                with SyncSessionLocal() as session:
                    upsert_complex_stats_sync(session, records_by_sigungu[sigungu_code])
            except Exception:
                logger.exception("%s 단지 집계 중 오류", sigungu_code)

    total_records = sum(len(recs) for recs in records_by_sigungu.values())
    logger.info(
